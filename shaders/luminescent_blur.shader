...
luminescent_blur.shader

combines a blur with the orginal image giving a "neon" look
...
void fragment() {
    vec4 col1 = textureLod(SCREEN_TEXTURE, SCREEN_UV, blur_amount);
    vec4 col2 = textureLod(SCREEN_TEXTURE, SCREEN_UV, 0.0);
    COLOR = mix(col1,col2, sharp_amount) * (hdr+1.0);
    }
